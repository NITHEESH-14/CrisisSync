...
export function RealtimeNotifications() {
    ...
    useEffect(() => {
        ...
        const isEnabled = localStorage.getItem('notifications_enabled') !== 'false'; // Default to true
        if (!isEnabled) return;

        // Query potentially new reports
        // We order by timestamp descending to get the newest
        const q = query(
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    ...
                    if (notifEnabled) {
                        // Check if it's the current user's report
                        const currentUser = JSON.parse(localStorage.getItem('user_profile') || '{}');
                        if (data.userId && data.userId === currentUser.id) {
                            return;
                        }

                        // CRITICAL: Check timestamp to ensure it's actually NEW.
                        // When a doc is deleted, 'limit(1)' pulls the next old doc, triggering 'added'.
                        // We filter out any report older than 60 seconds from NOW.
                        const reportTime = new Date(data.timestamp).getTime();
                        const now = Date.now();
                        const diffSeconds = (now - reportTime) / 1000;

                        // If report is older than 60 seconds (generous buffer for clock skew), ignore it.
                        // Ideally 30s is enough for "Live".
                        if (diffSeconds > 60) {
                            return;
                        }
                        ...
                        if ("Notification" in window && Notification.permission === "granted") {
                            new Notification("CrisisSync Emergency", {
                                ...
                                tag: data.id // Prevents duplicate notifs if updated
                            });
                        }
    ...
    const handleToastClick = () => {
        ...
        // Assuming /dashboard is the intended route, or / if that's the main view.
        // Based on file list, 'dashboard' dir exists, but 'page.tsx' is likely the main home.
        // User asked for "Responder Dashboard". Often '/' IS the dashboard.
        // If /dashboard exists, use it.
